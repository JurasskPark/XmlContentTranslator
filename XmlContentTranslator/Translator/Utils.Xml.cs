    ...
    public static partial class XmlUtils
    {
        ...
        public static string XmlValToStr(object? value)
        {
            if (value == null)
                ...
                return XmlUtils.ToLowerString(boolValue);
        ...
        {
            XmlDocument xmlDocument = parentXmlNode.OwnerDocument ?? throw new InvalidOperationException("Parent XML node must belong to a document.");
            XmlElement xmlElem = xmlDocument.CreateElement(elemName);
            string val = XmlValToStr(innerText);
            if (!string.IsNullOrEmpty(val))
                xmlElem.InnerText = val;
            return (XmlElement?)parentXmlNode.AppendChild(xmlElem) ?? xmlElem;
        }
        ...
        {
            XmlDocument xmlDocument = parentXmlNode.OwnerDocument ?? throw new InvalidOperationException("Parent XML node must belong to a document.");
            XmlElement paramElem = xmlDocument.CreateElement("Option");
            ...
            return (XmlElement?)parentXmlNode.AppendChild(paramElem) ?? paramElem;
        }
        ...
        public static XmlElement? GetOptionElem(this XmlNode parentXmlNode, string optionName)
        {
            ...
            return xmlNodes != null && xmlNodes.Count > 0 ? xmlNodes[0] as XmlElement : null;
        }
        ...
        {
            return DateTime.SpecifyKind(Xml.Shared.XmlUtils.GetChildAsDateTime(parentXmlNode, childNodeName, DateTime.MinValue), kind);
        }
        ...
        {
            return Xml.Shared.XmlUtils.GetChildAsDateTime(parentXmlNode, childNodeName, DateTime.MinValue);
        }
        ...
        {
            return Xml.Shared.XmlUtils.GetChildAsTimeSpan(parentXmlNode, childNodeName, TimeSpan.Zero);
        }
        ...
        {
            return DateTime.SpecifyKind(Xml.Shared.XmlUtils.GetAttrAsDateTime(xmlElem, attrName, DateTime.MinValue), kind);
        }
        ...
        {
            return Xml.Shared.XmlUtils.GetAttrAsDateTime(xmlElem, attrName, DateTime.MinValue);
        }
        ...
        {
            return Xml.Shared.XmlUtils.GetAttrAsTimeSpan(xmlElem, attrName, TimeSpan.Zero);
        }
        ...
        {
            return new FormatException(paramName);
        }
    }
}
